...
use App\AttendanceLine;

class AttendanceLineController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
      ...
      if($request->update == 1){
        ...
        foreach($details as $detail){
            if($detail->user_id == $request->user_id){
              if(strpos($detail->start,'T') !== false){
                $result[$loop]["name"] = $detail->name;
                $result[$loop]["start"] = substr(str_replace("T","  ",$detail->start), 0, -3);
                $result[$loop]["finish"] = $detail->finish;
                $result[$loop]["start_comment"] = $detail->start_comment;
                $result[$loop]["finish_comment"] = $detail->finish_comment;
                $result[$loop]["id"] = $detail->id;
                $loop++;
              }else{
                $result[$loop]["name"] = $detail->name;
                $result[$loop]["start"] = $detail->start;
                $result[$loop]["finish"] = $detail->finish;
                $result[$loop]["start_comment"] = $detail->start_comment;
                $result[$loop]["finish_comment"] = $detail->finish_comment;
                $result[$loop]["id"] = $detail->id;
        ...
        $view = 'voyager::attendance-lines.browse';

        // dd($result);

        return Voyager::view($view,compact('result'));
    }

    public function editor(Request $request)
    {
      $details = AttendanceLine::all();
      $detail = $details->where('id', '=', $request->id)->first();
      $result = [];
      $result["name"] = $detail->name;
      $result["start"] = $detail->start;
      $result["finish"] = $detail->finish;
      $result["start_comment"] = $detail->start_comment;
      $result["finish_comment"] = $detail->finish_comment;
      $result["id"] = $detail->id;
      $result["user_id"] = $detail->user_id;

      $view = 'voyager::attendance-lines.edit';
    ...
    //           $result[$loop]["name"] = $detail->name;
    //           $result[$loop]["start"] = substr(str_replace("T","  ",$detail->start), 0, -3);
    //           $result[$loop]["finish"] = str_replace("T","  ",$detail->finish);
    //           $result[$loop]["start_comment"] = $detail->start_comment;
    //           $result[$loop]["finish_comment"] = $detail->finish_comment;
    //           $result[$loop]["id"] = $detail->id;
    //           $loop++;
